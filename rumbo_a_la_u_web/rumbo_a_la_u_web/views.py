...
def transbankpay_transbank_commit(tokenws):
    ...
    url = "https://webpay3gint.transbank.cl/rswebpaytransaction/api/webpay/v1.2/transactions/{0}".format(
        tokenws)
    # CABECERA SOLICITADA POR TRANSBANK
    headers = transbankpay_header_request_transbank()
...
def transbankpay_transbank_reverse_or_cancel(tokenws: str = "", amount: int = None):
    ...
    print('data: ', data)
    # DEFINICIÓN DE URL DE TRANSBANK PARA CONFIRMAR UNA TRANSACCIÓN
    url = "https://webpay3gint.transbank.cl/rswebpaytransaction/api/webpay/v1.2/transactions/{0}/refunds".format(
        tokenws)
    # CABECERA SOLICITADA POR TRANSBANK
    headers = transbankpay_header_request_transbank()
    # INVOCACIÓN POR GET A API REST QUE CONFIRMA UNA TRANSACCIÓN EN TRANSBANK
    ...
    if tokenws is not None:
        ...
        if status == 'AUTHORIZED' and response_code == 0:
            ...
            transaction_date = dt.datetime.strptime(
                response['transaction_date'], '%Y-%m-%dT%H:%M:%S.%fZ')
            ...
            carts, quantities, total, products_cart = load_cart(
                customer_id=int(response['session_id']))
...
def generate_signature(sdk_key, sdk_secret, meeting_number, role):
    ...
    data = base64.b64encode(
        json.dumps({'meetingNumber': meeting_number,
