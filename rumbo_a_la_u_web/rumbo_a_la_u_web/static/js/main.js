...
// main.js (o tu archivo JavaScript principal global)

document.addEventListener('DOMContentLoaded', function () {
  // Función para actualizar el carrito en el encabezado
  function updateCartHeader() {
    // Obtener el número total de elementos del carrito desde "Mis compras"
    const totalItems = document.querySelectorAll('.ms-woocommerce-cart-form-wrapper .cart_item').length;

    // Actualizar el texto del encabezado del carrito
    const cartHeading = document.querySelector('.cart-heading');
    if (cartHeading) {
      cartHeading.textContent = `MY CART (${totalItems} ITEMS)`;
    }
  }
  ...
  // Eventos para escuchar cambios en el carrito en "Mis compras"
  // Aquí deberías agregar los listeners para los eventos relevantes que afectan el carrito en "Mis compras"

  // Ejemplo hipotético de cómo se podría escuchar un evento de eliminación de producto
  const deleteButtons = document.querySelectorAll('.ms-woocommerce-cart-form-wrapper .remove');
  deleteButtons.forEach(button => {
    button.addEventListener('click', function (event) {
      // Lógica para eliminar el producto del carrito en "Mis compras"
      event.currentTarget.closest('.cart_item').remove();

      // Después de eliminar, actualizar el carrito en el encabezado
      updateCartHeader();
    });
  });
});
